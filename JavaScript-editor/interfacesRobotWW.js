class RobotIWW {
    ...
    onmessage= function(e){
      switch (e.data.message) {
        ...
        case "sensor":
          ...
          }else if(e.data.function == "getMinorDistance" || e.data.function == "isTouching" ){
            sensors.distanceArray = e.data.parameter;
  ...
  async isTouching() {
    postMessage({message:"sensor",function:"isTouching"});
    this.finished.status=false;
    while(!this.finished.status){
      await this.sleep(0.1);
    }
  ...
  async getMinorDistance() {
    postMessage({message:"sensor",function:"getMinorDistance"});
    this.finished.status=false;
    while(!this.finished.status){
      await this.sleep(0.1);
    }
